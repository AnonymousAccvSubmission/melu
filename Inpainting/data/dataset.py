...
def make_dataset(dir):
    if os.path.isfile(dir):
        ...
        images = [os.path.join(dir, x) for x in images if is_image_file(x)]
...
class InpaintDataset(data.Dataset):
    def __init__(self, data_root, prior_root, mask_root, mask_config={}, data_len=-1, image_size=[256, 256], is_train=False):
    ...
    def __getitem__(self, index):
        ...
        # data augmentation
        #similar augmentations should be used. See what other networks are using.
    ...
    def get_mask(self, index=0):
        ...
        elif self.mask_mode == 'external':
            path = self.imgs[index]
            mask_name = path.split('/')[-1]
            mask = Image.open(os.path.join(self.mask_root, mask_name)).convert('L')
            return mask
        else:
